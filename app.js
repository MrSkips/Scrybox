...
(() => {
  ...
  const state = {
    ...
    // Dark ("grimoire") is the default look now — a flat white Scryfall
    // clone was the whole complaint that started the redesign, so the
    // richer, moodier theme is what a first-time load shows. Anyone who
    // explicitly unchecks the toggle gets that choice persisted via
    // persistAppSettings() as always, same as any other setting.
  ...
  const retryNoReuploadBtn = $('#retryNoReuploadBtn');
  ...
  // Shared failure handler for the many fire-and-forget localforage.setItem()
  // persistence calls scattered through the app (scratchpad, presets, AI
  // settings, etc.). These used to fail completely silently — a full
  // IndexedDB (private browsing, storage quota exceeded) meant a save
  // just silently didn't happen, with no trace in the console and no way
  // for the user to know their change wouldn't survive a reload. Now every
  // failure is logged, and the first one in a session also surfaces a
  // brief, low-key heads-up in the search bar's status area so the user
  // has a chance to notice before losing work.
  let storageFailureWarned = false;
  function logStorageFailure(what, err) {
    console.error(`Scrybox: failed to save ${what} to local storage`, err);
    if (storageFailureWarned) return;
    storageFailureWarned = true;
    if (syncStatusEl) {
      syncStatusEl.textContent = "Heads up: your browser blocked a local save (private browsing or storage full?). Recent changes may not survive a reload.";
      setTimeout(() => {
        if (syncStatusEl.textContent.startsWith('Heads up:')) syncStatusEl.textContent = '';
      }, 7000);
    }
  }

  function persistExcludedBinders() {
    localforage.setItem(EXCLUDED_BINDERS_KEY, Array.from(state.excludedBinders)).catch((err) => logStorageFailure('excluded binders', err));
  }

  // ---------------------------------------------------------------------
  // Navigation & view switching (Collection/Value/Stats/Decks/Binders/
  // Deck Builder top-nav tabs — each show* function hides every other
  // screen and shows its own)
  // ---------------------------------------------------------------------
  ...
  // Keeps the sort-direction button's visible text and its aria-label in
  // sync in one place, since it's set from three different spots (Value
  // tab shortcut, the button's own click handler, and the table header
  // click-to-sort handler).
  function setSortDirBtnLabel() {
    const btn = $('#sortDirBtn');
    const asc = state.sortDir === 'asc';
    btn.textContent = asc ? 'Asc ↑' : 'Desc ↓';
    btn.setAttribute('aria-label', `Toggle sort direction, currently ${asc ? 'ascending' : 'descending'}`);
  }
  ...
  async function ensureSetsMeta() {
    ...
    state.setsMeta = map;
    localforage.setItem(SETS_META_KEY, { map, fetchedAt: Date.now() }).catch((err) => logStorageFailure('the cached set-completeness data', err));
    return map;
  }
  ...
  function bindUploadEvents() {
    ...
    retryNoReuploadBtn.addEventListener('click', () => {
      if (!state.lastParsedUpload) return;
      enrichAndStore(state.lastParsedUpload.groups, state.lastParsedUpload.binders);
    });
  }
  ...
  async function resetCollection() {
    ...
    try {
      ...
      // We still reset in-memory state below either way, but if the clear
      // itself failed, the old data may still be sitting in storage and
      // reappear on next load — worth knowing about.
      logStorageFailure('clearing the old collection', err);
    }
    ...
    showUndoToast(`Cleared ${snapshotCollection.length.toLocaleString()} cards.`, async () => {
      ...
      try {
        ...
        logStorageFailure('restoring the collection after Undo', err);
      }
  ...
  async function processRows(rows) {
    ...
    // Cached so a Scryfall network/rate-limit failure can be retried without
    // re-selecting the file from disk — see retryNoReuploadBtn.
    state.lastParsedUpload = { groups: groupsArr, binders };
    await enrichAndStore(groupsArr, binders);
  }
  ...
  function slowDownBatchPacing() {
    ...
    try { localStorage.setItem(SCRYFALL_PACING_KEY, String(currentBatchDelayMs)); } catch (e) { console.warn('Scrybox: could not persist learned Scryfall pacing (non-fatal, just resets to the base pace next reload)', e); }
  }
  ...
  async function enrichAndStore(groups, binders) {
    ...
    retryNoReuploadBtn.classList.add('hidden');
    ...
    if (toFetch.length) {
      try {
        ...
        if (partial.length) {
          ...
          } catch (storeErr) { logStorageFailure('partial import progress', storeErr); /* best-effort — the in-memory state still has it for this session */ }
          progressNote = ` Saved progress on ${partial.length.toLocaleString()} card${partial.length === 1 ? '' : 's'} fetched before this happened.`;
        }
        // Reuse the parsed CSV already sitting in memory instead of making
        // them re-select the file — that was the main friction point.
        if (state.lastParsedUpload) {
          progressNote += ' Wait a bit, then click "Retry without re-uploading" below.';
          retryNoReuploadBtn.classList.remove('hidden');
        }
        if (err.isRateLimit) {
    ...
    try {
      ...
      console.error('Scrybox: failed to save the imported collection to local storage', err);
  ...
  async function refreshPrices() {
    ...
    try {
      ...
      logStorageFailure('refreshed prices', err);
    }
  ...
  function exportResultsForDeckbuilding() {
    const groups = getGroupedAndSortedResults(state.filtered);
    if (!groups.length) {
    ...
    downloadTextFile(text, buildExportFilename('txt'));
  }
  ...
  function buildExportFilename(extension) {
    ...
    return parts.join('-') + '.' + (extension || 'txt');
  }

  // Downloads whatever's currently showing (respecting the search box and
  // every sidebar filter) as a ManaBox-compatible CSV, so it can be
  // re-imported into Scrybox (or ManaBox itself) as its own collection/list.
  // One row is emitted per binder the card is tracked in (split by foil
  // status), plus one "unassigned" row for any quantity not tied to a
  // binder — this mirrors how Scrybox's own importer reconstructs
  // card._binders on the way in. Condition and Purchase price are left
  // blank: Scrybox's importer recognizes those columns but never actually
  // stores their values, so the original data no longer exists to export.
  function exportResultsAsManaBoxCsv() {
    const groups = getGroupedAndSortedResults(state.filtered);
    if (!groups.length) {
      syncStatusEl.textContent = 'Nothing to export — adjust your filters/search so some cards are showing first.';
      setTimeout(() => { syncStatusEl.textContent = ''; }, 3500);
    ...
    const rows = [];
    const pushRow = (card, binderName, binderType, foil, qty) => {
      if (!qty) return;
      rows.push({
        'Name': card.name,
        ...
        // "list" is a reserved substring Scrybox's own importer uses to
        // skip virtual ManaBox "Lists" on the way back in — avoid it here
        // so every row we write actually round-trips.
    ...
    groups.forEach((g) => {
      g.versions.forEach((v) => {
        // Every version's own (unadjusted) collection record — not the
        // pull-adjusted one used for on-screen availability — so exported
        // quantities reflect what's really in the collection.
        const card = state.collection.find((c) => c.id === v.id) || v;
        const binders = card._binders || [];
        let trackedFoil = 0;
        let trackedNonfoil = 0;
        binders.forEach((b) => {
          trackedFoil += b.qtyFoil || 0;
          trackedNonfoil += b.qtyNonfoil || 0;
          const type = b.type === 'deck' ? 'deck' : (b.type === 'binder' ? 'binder' : 'other');
          pushRow(card, b.name, type, true, b.qtyFoil || 0);
          pushRow(card, b.name, type, false, b.qtyNonfoil || 0);
        });
        const unassignedFoil = Math.max(0, (card.qtyFoil || 0) - trackedFoil);
        const unassignedNonfoil = Math.max(0, (card.qtyNonfoil || 0) - trackedNonfoil);
        pushRow(card, 'Scrybox Collection', 'binder', true, unassignedFoil);
        pushRow(card, 'Scrybox Collection', 'binder', false, unassignedNonfoil);
      });
    });

    if (!rows.length) {
      syncStatusEl.textContent = 'Nothing to export — no owned quantities found for the current results.';
      setTimeout(() => { syncStatusEl.textContent = ''; }, 3500);
      return;
    }

    const csv = Papa.unparse(rows);
    downloadTextFile(csv, buildExportFilename('csv'), 'text/csv');
  }
  ...
  // Memoized grouping+sorting: `renderResults`, both export functions, and
  // the Value tab total all independently want "the current search results,
  // grouped by name and sorted" — without caching, that's a full O(n) group
  // plus O(n log n) sort over the *entire* filtered set (not just the
  // visible page) recomputed from scratch every time any of them run, even
  // when nothing about the underlying results actually changed (e.g.
  // toggling Grid/Table view, or exporting right after a render). Grouping
  // is cached on the identity of `state.filtered` (a fresh array every
  // search, so reference equality is a safe and free check); sorting is
  // cached separately since only sortBy/sortDir affect it, not the source
  // data — so changing sort direction doesn't force a full regroup.
  let groupCache = { source: null, groups: null };
  let sortCache = { groups: null, sortBy: null, sortDir: null, sorted: null };
  function getGroupedAndSortedResults(cards) {
    if (groupCache.source !== cards) {
      groupCache = { source: cards, groups: groupCardsByName(cards) };
      sortCache = { groups: null, sortBy: null, sortDir: null, sorted: null }; // invalidate stale sort
    }
    if (sortCache.groups !== groupCache.groups || sortCache.sortBy !== state.sortBy || sortCache.sortDir !== state.sortDir) {
      sortCache = {
        groups: groupCache.groups,
        sortBy: state.sortBy,
        sortDir: state.sortDir,
        sorted: sortGroups(groupCache.groups)
      };
    }
  ...
  function renderTableRow(group) {
    ...
    // Left-edge rarity stripe (see .results-table tbody tr[data-rarity] in
    // CSS) — same rarity-gem color convention as the grid tiles, so rarity
    // is scannable in table view too instead of being plain lowercase text.
    tr.dataset.rarity = card.rarity || 'common';
    ...
    tr.innerHTML =
      ...
      `<td><span class="rarity-label rarity-${escapeHtml(card.rarity || 'common')}">${escapeHtml(card.rarity || '')}</span></td>` +
      `<td>${qty}</td>` +
  ...
  function renderCardTile(group) {
    ...
    // Small rarity gem in the corner, same color convention real MTG cards
    // use (common/uncommon/rare/mythic) — see .rarity-gem in CSS.
    const gem = document.createElement('span');
    gem.className = 'rarity-gem rarity-' + (card.rarity || 'common');
    gem.title = card.rarity ? card.rarity.charAt(0).toUpperCase() + card.rarity.slice(1) : '';
    ...
    addBtn.setAttribute('aria-label', `Add ${card.name} to Deck Builder`);
    addBtn.textContent = '+';
  ...
  function renderPagination(totalPages) {
    [paginationBottom, paginationTop].forEach((container) => {
      ...
      const makeBtn = (label, page, active, ariaLabel) => {
      ...
      if (state.page > 1) container.appendChild(makeBtn('‹ Prev', state.page - 1, false, 'Previous page'));
      const windowSize = 5;
      let from = Math.max(1, state.page - Math.floor(windowSize / 2));
      let to = Math.min(totalPages, from + windowSize - 1);
      from = Math.max(1, to - windowSize + 1);
      for (let p = from; p <= to; p++) container.appendChild(makeBtn(String(p), p, p === state.page, `Page ${p}${p === state.page ? ', current page' : ''}`));
      if (state.page < totalPages) container.appendChild(makeBtn('Next ›', state.page + 1, false, 'Next page'));
    });
  }
  ...
  function openModal(group, selectedIndex) {
    ...
    modalRarityEl.className = 'rarity-label rarity-' + (card.rarity || 'common');
    const ownedParts = [];
  ...
  function persistScratchpad() {
    localforage.setItem(SCRATCHPAD_KEY, state.scratchpad).catch((err) => logStorageFailure('the Deck Builder scratchpad', err));
  }
  ...
  // Recognized non-card noise lines that decklists commonly include but
  // that aren't themselves cards: blank lines (already dropped), "//" or
  // "#" comments, and bare section headers (with or without a trailing
  // colon) that sites like Moxfield/Archidekt/MTGGoldfish export. Without
  // this, a line like "Commander" or "// Ramp" used to become a bogus
  // ghost card. A leading "SB:" (sideboard marker) is stripped rather than
  // skipped, since the scratchpad doesn't distinguish main/sideboard —
  // treating it as a normal card is more useful than dropping it.
  const DECKLIST_SECTION_HEADERS = /^(commander|deck|maindeck|main\s*deck|mainboard|sideboard|maybeboard|companion|lands?|creatures?|spells?|other)\s*:?\s*$/i;
  function parseDecklistText(text) {
    return (text || '').split('\n')
      .map((line) => line.trim())
      .filter(Boolean)
      .filter((line) => !line.startsWith('//') && !line.startsWith('#'))
      .filter((line) => !DECKLIST_SECTION_HEADERS.test(line))
  ...
  // `nameIndex` (lowercased card name -> matching card records) lets a bulk
  // decklist import build this map once, up front, instead of filtering
  // the entire collection again for every single copy of every line —
  // meaningful at collection sizes worth caring about, where an import
  // used to be an O(decklist lines × copies × collection size) scan.
  function buildCollectionNameIndex() {
    const idx = new Map();
    state.collection.forEach((c) => {
  ...
  function getAvailableLocationsForName(name, nameIndex) {
    const lower = name.toLowerCase();
    const cards = nameIndex ? (nameIndex.get(lower) || []) : state.collection.filter((c) => c.name.toLowerCase() === lower);
    const combined = [];
    cards.forEach((card) => {
  ...
  function importDecklistText(text) {
    ...
    entries.forEach((entry) => {
      ...
      for (let i = 0; i < entry.qty; i++) {
        const combined = getAvailableLocationsForName(entry.name, nameIndex);
  ...
  function removeFromScratchpad(key) {
    const removed = findScratchItem(key);
    if (!removed) return;
    ...
    showUndoToast(`Removed ${removed.name} (${removed.qty}x).`, () => {
      // Restore the exact item, pulls included, so anything it had taken
      // out of a binder goes right back to being "pulled" instead of
      // silently returning to availability.
      if (!findScratchItem(key)) state.scratchpad.push(removed);
      persistScratchpad();
      renderScratchpad();
      if (card) refreshAfterPullChange(card); else runSearch();
    });
  ...
  function downloadScratchpadTxt() {
    downloadTextFile(scratchpadExportText(), `scrybox-scratchpad-${timestampSlug()}.txt`);
  }
  ...
  function downloadScratchpadPullListTxt() {
    downloadTextFile(scratchpadPullListText(), `scrybox-pull-list-${timestampSlug()}.txt`);
  }
  ...
  function renderScratchpadTile(item) {
    ...
    minus.type = 'button';
    minus.textContent = '−';
    // When a real card is stored in more than one place, releasing a copy
    // always gives back the most recently pulled one first (last in, first
    // out) — deterministic, but worth spelling out since it's not obvious
    // which specific binder/deck gets it back.
    minus.title = (!item.ghost && item.pulls && item.pulls.length > 1)
      ? 'Remove one (gives back the most recently pulled copy first)'
      : 'Remove one';
    ...
    remove.setAttribute('aria-label', `Remove all copies of ${item.name}`);
  ...
  function persistPresets() {
    localforage.setItem(PRESETS_KEY, state.presets).catch((err) => logStorageFailure('your saved searches', err));
  }
  ...
  async function handleRestoreFile(file) {
    ...
    try {
      ...
      logStorageFailure('the restored backup', err);
    }
  ...
  function persistAiSettings() {
    localforage.setItem(AI_SETTINGS_KEY, state.aiSettings).catch((err) => logStorageFailure('your AI settings', err));
  }

  function persistAiGrades() {
    localforage.setItem(AI_GRADES_KEY, state.aiGrades).catch((err) => logStorageFailure('AI deck grades', err));
  }
  ...
  function persistAppSettings() {
    localforage.setItem(APP_SETTINGS_KEY, state.appSettings).catch((err) => logStorageFailure('app settings (theme, etc.)', err));
  }
  ...
  // Single source of truth for both the build checklist AND the quick-jump
  // tag chips above it — used to duplicate this list by hand in two places.
  // filterKind/filterValue is what jumpToDeckBuilderFilter needs to build
  // the chip's search jump. `target` numbers below are calibrated for a
  // 99-card singleton Commander deck; renderDeckBuilderChecklist scales
  // them proportionally against the "Deck size" input so this isn't
  // permanently Commander-only (a 60-card deck gets roughly 60/99 of each
  // target, etc.) — it's a rough guideline either way, not a hard rule.
  const DECK_BUILDER_TARGETS = [
    { key: 'lands', label: 'Lands', target: 36, test: (c) => (c.type_line || '').toLowerCase().includes('land'), filterKind: 'type', filterValue: 'land' },
    { key: 'ramp', label: 'Ramp', target: 10, test: (c) => c._playstyles && c._playstyles.has('ramp'), filterKind: 'playstyle', filterValue: 'ramp' },
    { key: 'draw', label: 'Card Draw', target: 10, test: (c) => c._tags && c._tags.has('cardadvantage'), filterKind: 'tag', filterValue: 'cardadvantage' },
    { key: 'removal', label: 'Removal', target: 10, test: (c) => c._tags && c._tags.has('removal'), filterKind: 'tag', filterValue: 'removal' },
    { key: 'wipes', label: 'Board Wipes', target: 3, test: (c) => c._tags && c._tags.has('boardwipe'), filterKind: 'tag', filterValue: 'boardwipe' },
    { key: 'protection', label: 'Protection', target: 5, test: (c) => c._tags && c._tags.has('protection'), filterKind: 'tag', filterValue: 'protection' }
  ];
  ...
  function renderDeckBuilderTags(commander) {
    ...
    const defs = DECK_BUILDER_TARGETS.map((t) => ({ label: t.label, kind: t.filterKind, value: t.filterValue }));
  ...
  function scaledDeckBuilderTarget(baseTarget) {
    const deckSize = state.appSettings.deckBuilderTargetSize || 99;
    return Math.max(1, Math.round(baseTarget * (deckSize / 99)));
  }
  ...
  function renderDeckBuilderChecklist() {
    ...
    DECK_BUILDER_TARGETS.forEach((t) => {
      ...
      const target = scaledDeckBuilderTarget(t.target);
      const met = count >= target;
      const pct = count ? Math.min(100, Math.max(4, Math.round((count / target) * 100))) : 0;
      const row = document.createElement('div');
      ...
      row.innerHTML =
        ...
        `<span class="dbc-count">${count}/${target}${met ? ' ✓' : ''}</span>`;
  ...
  function renderScratchCommanderRow() {
    ...
    if (commander) {
      ...
      // Don't stomp on the deck-size input while the user has it focused —
      // this render function fires on nearly every scratchpad mutation, and
      // resetting the field mid-edit would be a frustrating fight against
      // whatever they're typing.
      if (document.activeElement !== deckSizeInput) {
        deckSizeInput.value = state.appSettings.deckBuilderTargetSize || 99;
      }
  ...
  function persistCommanderSynergyTags() {
    localforage.setItem(COMMANDER_SYNERGY_TAGS_KEY, state.commanderSynergyTags).catch((err) => logStorageFailure('AI-suggested synergy tags', err));
  }

  // Synergies only depend on the commander + the owned collection (and
  // saved AI tags) — NOT on scratchpad contents — but renderScratchCommanderRow
  // re-runs on every single scratchpad add/remove/qty change, which used to
  // mean a full O(categories × collection) rescan (or several live AI query
  // re-runs) on every click. Cache the result and only recompute when the
  // commander, the collection, or the saved tags actually changed.
  let commanderSynergiesCacheKey = null;
  let commanderSynergiesCacheCollection = null;
  let commanderSynergiesCacheResult = null;
  let commanderSynergyTagsVersion = 0;
  ...
  // against the current collection below, never cached stale (beyond the
  // memoization above, which is invalidated on any real change).
  async function computeCommanderSynergyTabs(commander) {
    // state.collection.length alone isn't a reliable "did the collection
    // change" signal — a re-sync that swaps N cards for a different N
    // cards (same count) wouldn't be caught by it. state.collection is
    // always reassigned to a brand-new array on sync/restore rather than
    // mutated in place, so comparing the array reference itself is both
    // cheap and actually correct.
    const cacheKey = `${commander.id}|${state.lastSyncedAt}|${commanderSynergyTagsVersion}`;
    if (commanderSynergiesCacheKey === cacheKey && commanderSynergiesCacheCollection === state.collection) {
    ...
    const result = { categories, source };
    commanderSynergiesCacheKey = cacheKey;
    commanderSynergiesCacheCollection = state.collection;
    commanderSynergiesCacheResult = result;
    return result;
  }
  ...
  async function renderCommanderSynergies(commander) {
    ...
    // `categories`/`t.matches` come straight out of the memoized cache above
    // (computeCommanderSynergyTabs), which is keyed on the commander +
    // collection + AI-tag version — deliberately NOT on scratchpad
    // contents, since re-scanning every category against the full
    // collection on every single add/remove would be the expensive
    // per-render cost that memoization exists to avoid. That means
    // `t.matches` can include cards you've since fully pulled into the
    // Deck Builder. Rather than mutate the cached array (which would make
    // a card that's later returned to storage never come back), filter
    // down to a fresh `availableMatches` array on every render instead —
    // cheap, since it's only scanning each category's already-small match
    // list, not the whole collection.
    categories.forEach((t) => {
      t.availableMatches = t.matches.filter((c) => getAvailableLocations(c).length > 0);
    });
    ...
    categories.forEach((t, idx) => {
      ...
      tab.type = 'button';
      tab.className = 'cmdr-syn-tab' + (source === 'ai' ? ' ai-sourced' : '') + (idx === state.commanderSynergiesActiveTab ? ' active' : '');
      tab.textContent = `${t.label} (${t.availableMatches.length})`;
      const titleParts = [];
      if (source === 'ai') titleParts.push('AI-suggested query');
      if (t.query) titleParts.push(t.query);
      if (titleParts.length) tab.title = titleParts.join(': ');
    ...
    if (!active.availableMatches.length) {
      const empty = document.createElement('p');
      empty.className = 'fg-hint';
      empty.textContent = active.matches.length
        ? `Every match for this theme is already in the Deck Builder.`
      ...
      shown.forEach((card) => {
        ...
        img.alt = card.name;
        tile.appendChild(img);
        // Same quick-add "+" the main Collection grid has, so a match can
        // be pulled straight into the Deck Builder without opening the
        // modal first — and since this whole panel re-renders off of
        // availableMatches (computed fresh every render, not cached), the
        // card actually disappears from this list the moment it's fully
        // pulled, instead of lingering here looking untouched.
        const addBtn = document.createElement('button');
        addBtn.type = 'button';
        addBtn.className = 'tile-add-btn';
        addBtn.title = 'Add to scratchpad';
        addBtn.setAttribute('aria-label', `Add ${card.name} to Deck Builder`);
        addBtn.textContent = '+';
        addBtn.addEventListener('click', (e) => { e.stopPropagation(); pickAndPull(card); });
        tile.appendChild(addBtn);
        const nameEl = document.createElement('div');
        nameEl.className = 'card-name';
        nameEl.textContent = card.name;
  ...
  function bindScratchCommanderEvents() {
    deckSizeInput.addEventListener('change', () => {
      const n = parseInt(deckSizeInput.value, 10);
      state.appSettings.deckBuilderTargetSize = (!isNaN(n) && n > 0) ? Math.min(250, n) : 99;
