...
(() => {
  ...
  function resetScryfallRateLimitState() { scryfallRateLimitSeen = false; }
  ...
  async function fetchAndBuildCards(groups) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      (data.data || []).forEach((card) => found.push(card));
      // Scryfall asks for 50-100ms between requests (~10/sec) — pacing at
      // the safer end of that after seeing rate-limit hiccups in practice.
  ...
  async function enrichAndStore(groups, binders) {
    ...
    if (toFetch.length) {
      try {
        ...
        if (err.isRateLimit) {
          setStatus(err.message + ' Nothing was changed.', true);
        } else {
          setStatus('Network error while contacting Scryfall (' + err.message + '). Nothing was changed — try again. If this keeps happening, check for an ad blocker or privacy extension blocking requests to api.scryfall.com (try disabling it just for this site), since Scryfall itself is a public API that works from any browser otherwise.', true);
        }
        return;
      }
    }
