...
A small, low-contrast version number (e.g. `v30`) sits in the bottom-right corner of the page — bump it (and the matching `?v=N` cache-busting query string on `style.css`/`app.js` in `index.html`) any time either file changes, so browsers don't serve a stale cached copy.
