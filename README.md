...
Once you upload your ManaBox CSV, your collection (plus binder/deck data) is cached in the browser's local storage. Reopening the app — even after restarting your computer — goes straight to your collection instead of asking you to re-upload. The header shows when it was last synced. Use **Upload / Resync** in the top nav any time you want to refresh from a new CSV export.

### Smart re-sync

Re-uploading a CSV doesn't wipe and re-fetch everything. It diffs against what's cached: unchanged cards are reused as-is, only genuinely new printings get fetched from Scryfall, and anything no longer in your CSV (sold/traded away) is dropped. The status message after a re-sync tells you how many were new/unchanged/removed. Scryfall requests retry automatically (up to 5 times, with backoff starting at 1.5s and doubling each attempt) if a request times out or gets rate-limited, instead of aborting the whole import — rate-limit responses from Scryfall sometimes come back without CORS headers, which browsers report as a generic blocked/network error rather than a readable "429," so the retry logic treats any failed request as a possible rate limit and backs off generously either way.
...
## Exporting results for AI deckbuilding

**Export results ↓** (next to 🛠 Deck Builder in the search bar) downloads whatever's currently shown — respecting the search box and every sidebar filter, color identity included — as a plain-text file with one line per card: quantity owned, name, mana cost, color identity, type line, power/toughness or loyalty, and full oracle text. It's meant to be handed straight to an AI (like Claude) to build a deck from, without it needing to look anything up.

Typical flow: pick your colors in **Color Identity** (Commander-style, so this also picks up every mono-colored and colorless card that fits), narrow further with any other filters or a search term if you want, then click **Export results ↓** and share the file.
