...
Scrybox has an MTG-flavored visual identity: headings use **Beleren Bold** (the actual Magic card-title typeface — it was already being loaded for the mana-symbol font but never applied anywhere until now), a warm parchment-toned light theme and a deep-navy/violet dark theme (the existing **Dark theme** toggle in Settings), and the same rarity-color convention real Magic cards use — common (gray), uncommon (silver-blue), rare (gold), mythic (orange) — now shown as a small colored gem in the corner of every card tile, a colored label in the table view and card modal, and a matching left-edge stripe on table rows.

A few other things from this pass:

- **Filters got reorganized.** Deck Status and Format Legality — previously stray controls sitting in the results toolbar — now live in the sidebar with the rest of the filters, where they belong.
- **Motion throughout.** Card tiles fade/slide in with a light staggered wave as the grid renders, the card modal pops in rather than just appearing, buttons have real hover/press feedback, and score/checklist bars animate when their values change. Everything respects your OS's "reduce motion" setting.
...
- `style.css` — MTG-flavored design system (see Design above)
