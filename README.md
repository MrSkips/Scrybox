...
If a large import does fail partway through (e.g. "batch 26 of 40"), Scrybox now saves whatever it fetched before the failure instead of throwing it away. Click **Upload / Resync** again with the same CSV once the rate limit clears — it'll skip everything already saved and only fetch what's still missing, so each retry gets further rather than starting over from card #1.
